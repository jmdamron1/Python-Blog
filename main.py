...
class Comment(db.Model):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    body: Mapped[str] = mapped_column(String(100000), nullable=False)

    post_id: Mapped[int] = mapped_column(ForeignKey("blog_post.id"))
    post = relationship("BlogPost", back_populates="comments", foreign_keys=post_id)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            flash(f"An account already exists with {form.email.data}!. Try logging in.")
            return redirect(url_for("login"))
        hashed_password = werkzeug.security.generate_password_hash(form.password.data, method='pbkdf2:sha256', salt_length=8)
    ...
    return render_template("register.html", form=form, flashes=get_flashed_messages())
...
def login():
    ...
    if form.validate_on_submit():
        ...
        elif werkzeug.security.check_password_hash(user.password, form.password.data):
            login_user(user)
            return redirect(url_for("get_all_posts"))
        else:
